...
//             doc.text(`Khuyến mãi: ${order.discountValue || 0}`, startX + 2 * columnWidth, totalY, { width: 2 * columnWidth, align: 'left' });
...
function generateInvoice(order, outputPath) {  
    return new Promise((resolve, reject) => {  
        ...
        function generateHeader(doc) {  
            ...
            doc.moveDown();  
            doc.text(`Số HĐ: ${order._id}`, { align: 'left' });  
            doc.text(`Ngày: ${new Date(order.createdAt).toLocaleDateString('vi-VN')}`, { align: 'left' });  
            doc.moveDown();  
        }  
        ...
        function generateBody(doc) {  
            ...
            order.items.forEach((product, index) => {  
                const rowY = headerY + (index + 1) * rowHeight;  
                doc.text(product.name, startX, rowY, { width: columnWidth, align: 'center' });  
                doc.text(formatCurrency(product.priceItem), startX + columnWidth, rowY, { width: columnWidth, align: 'center' });  
                doc.text(product.quantity.toString(), startX + 2 * columnWidth, rowY, { width: columnWidth, align: 'center' });  
                doc.text(formatCurrency(product.totalPrice), startX + 3 * columnWidth, rowY, { width: columnWidth, align: 'center' });  
            ...
            doc.text(`Tổng tiền: ${formatCurrency(subtotal)}`, startX + 2 * columnWidth, totalY, { width: 2 * columnWidth, align: 'left' });  
            doc.text(`Khuyến mãi: ${formatCurrency(order.discountValue || 0)}`, startX + 2 * columnWidth, totalY + 20, { width: 2 * columnWidth, align: 'left' });  
            doc.text(`Tổng cộng: ${formatCurrency(order.totalPrice)}`, startX + 2 * columnWidth, totalY + 40, { width: 2 * columnWidth, align: 'left' });  
            doc.moveDown();  
        }  
